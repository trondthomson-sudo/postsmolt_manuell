...
rotasjon (typisk 3 eller 4, én per runde i året), hver med sitt eget:
  ...
  - Antall uker kohorten star i tank 2/3 (vekstkar) for salg
  - Antall uker vask av tank 1 OG antall uker vask av vekstkar - begge
...
ny kohort overfores fra tank 1 for forrige kohort er levert og karet rukket
...
def build_manual_schedule(cfg, growth_tables: GrowthTables | None = None):
    ...
    (liste - vasketid for VEKSTKAR, ogsa individuell per oppskrift, slik at
    ...
    while True:
        ...
        # ---- Kollisjonssjekk: rekker vekstkarene a bli levert + vasket for
...
def _assemble_wide_table(cfg, cohorts, generations, n_weeks_total):
    ...
    for gid, info in generations.items():
        ...
        for k in range(info["tank1_cleaning_weeks"]):
            ...
            if wk < n_weeks_total and wk not in tanks["tank1"]:
                ...
                # karet (f.eks. "G2"), sa det alltid er entydig hvilken
    ...
    # ---- Vekstkar (tank 2/3): fyll HELE gapet mellom levering og neste
    ...
    # gapet er fortsatt et kar som star tomt/vasket, ikke en egen "ledig,
    ...
    for idx, (gid, info) in enumerate(sorted_gens):
        ...
        if idx + 1 < len(sorted_gens):
            ...
            # konfigurerte vasketiden for vekstkar - marker cellene tydelig
        ...
        # akkurat dette karet (f.eks. "G2") - viktig fordi vaskeblokken ofte
    ...
    # ---- Idriftsettelsesvask: fyll ogsa vekstkarenes ALLER FØRSTE bruk med
    # "Rengjoring" rett før første overføring - selv om det ikke fantes noen
    # forrige kohort å vaske bort etter, klargjøres et nytt kar likevel før
    ...
    if sorted_gens:
        ...
        kohort_lbl = f"(→{first_gid})"  # ingen kohort dro fra karet - vask FOR forste ankomst
        for tname in ("tank2", "tank3"):
