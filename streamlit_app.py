"""
streamlit_app.py - postsmolt, MANUELT TIDSSTYRT (bruker-definert oppskrift, 3 kar)
-----------------------------------------------------------------------------------
...
st.set_page_config(page_title="Hexacage - postsmolt, manuell modell (3x/år)", layout="wide")
st.title("Hexacage produksjonsplan - manuelt tidsstyrt postsmolt-modell (3 kar, 3x runder/år default)")
st.caption(
    "Her styrer du selv timingen for en 'mal-kohort' - når den settes inn, "
    "hvor lenge den står i hvert kar, når karene vaskes - og modellen "
...
with st.sidebar:
    ...
    for i in range(n_batches_in_rotation):
        ...
        growout_i = bc2.number_input(
            "Uker i vekstkar", min_value=1, max_value=52,
        ...
        clean_i = bc3.number_input(
            ...
            help="Vasketid for tank 1 etter denne oppskriften er overført til "
                 "vekstkar - fleksibel 1-5 uker, slik at du kan justere summen "
                 "for hele rotasjonen til å treffe akkurat 52 uker.",
        )
        growout_clean_i = bc4.number_input(
            "Vask vekstkar", min_value=1, max_value=5,
            value=max(1, min(5, int(d_growout_clean))), step=1, key=f"growout_clean_batch_{i}",
            help="Vasketid for vekstkar (tank 2/3) etter denne oppskriften er "
    ...
    st.divider()
    st.header("Kar / lokalitet")
    tank_volume_m3 = _auto_format_number_input(
        "Kar-volum (m3, alle like)", key="tank_volume_text",
    ...
    st.caption("3 kar totalt: tank 1 (nursery) + tank 2/3 (vekstkar, 50/50-splitt).")
...
_recipe_lines = [
    ...
    f"+ {meta['batch_growout_weeks'][i]} uker vekstkar + {meta['batch_growout_cleaning_weeks'][i]} uker vask vekstkar"
    for i in range(meta["n_batches_in_rotation"])
]
if meta["kollisjon_i_oppskriften"]:
    recipe_info_placeholder.warning(
        "Denne rotasjonen kolliderer for minst én oppskrift: vekstkarene "
...
if n_kollisjon:
    st.warning(
        ...
        f"overføring fra tank 1 skjer før vekstkarene fra forrige kohort er "
    ...
    st.caption("Ingen kollisjon: vekstkarene er levert og vasket i god tid før neste overføring, for alle kohorter.")
...
for gid, info in complete_gens.items():
    ...
    cohort_rows_all.append({
        ...
        "Vekstkar-uker": info["growout_weeks"],
        "Vask vekstkar-uker": info["growout_cleaning_weeks"],
...
ax1.set_title("Ståande biomasse per kar")
...
# ---- 2. Tetthet per kar (kg/m3), med valgt tetthetstak ----
...
if max_dens_overall >= max_density:
    ...
    st.caption(f"Høyeste tetthet i valgt periode: {fmt_float(max_dens_overall,1)} kg/m³ - under taket ({fmt_float(max_density,0)} kg/m³) i alle kar.")

# ---- 3. Fiskestørrelse per kar (g) ----
...
for i, t in enumerate(tank_labels):
    ...
    ax3.plot(dates_x, vekt, label=t, color=TANK_COLORS.get(t, "#888888"),
              linewidth=2.4, linestyle=LINESTYLES[i % len(LINESTYLES)])
    # Merk sluttvekten (siste uke med fisk i karet, rett før neste
...
ax3.set_title("Fiskestørrelse per kar")
...
st.pyplot(fig3)
st.caption(
    "Linjene brytes i rengjørings-/ledig-uker (ingen fisk i karet da). "
...
# Excel-eksporten: felt som rader, uker som kolonner, farge per kar)
# ----------------------------------------------------------------------
...
st.caption(
    ...
    "rengjøring (også vekstkarenes idriftsettelsesvask helt i starten, før "
    ...
    "vekstkar rekker å bli fullført - karet vises da bare tomt så lenge det "
